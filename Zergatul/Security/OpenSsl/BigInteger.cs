...
namespace Zergatul.Security.OpenSsl
{
    class BigInteger
    {
        ...
        public static BigInteger Random(BigInteger value, SecureRandom random)
        {
            ...
            if (OpenSsl.BN_is_zero(value.Pointer) == 1)
                throw new InvalidOperationException();

            byte[] bytes = new byte[OpenSsl.BN_num_bytes(value.Pointer)];
            if (OpenSsl.BN_bn2bin(value.Pointer, bytes) <= 0)
            ...
            IntPtr bn = OpenSsl.BN_bin2bn(result, result.Length, IntPtr.Zero);
            if (bn == IntPtr.Zero)
                throw new OpenSslException();
