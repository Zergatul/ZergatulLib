...
namespace Zergatul.Security.OpenSsl
{
    class AES : SymmetricCipher
    {
        ...
        public override int Update(byte[] input, int inputLength, byte[] output)
        {
            ...
            if (OpenSsl.EVP_CipherUpdate(_ctx, output, ref outputLength, input, inputLength) != 1)
        ...
        public override int DoFinal(byte[] output)
        {
            ...
            if (_enc)
            {
                ...
                int outputLength = output.Length;
                if (OpenSsl.EVP_CipherUpdate(_ctx, output, ref outputLength, pad, pad.Length) != 1)
                    throw new OpenSslException();
                return outputLength;
            }
            else
        ...
        private void Init(byte[] key, bool enc, SymmetricCipherParameters parameters)
        {
            ...
            switch (_keyLen)
            {
                case 16:
                    ...
                    {
                        case BlockCipherMode.ECB: type = OpenSsl.EVP_aes_128_ecb(); break;
                        case BlockCipherMode.CBC: type = OpenSsl.EVP_aes_128_cbc(); break;
                        default:
                ...
                case 24:
                    switch (parameters.Mode)
                    {
                        case BlockCipherMode.ECB: type = OpenSsl.EVP_aes_192_ecb(); break;
                        case BlockCipherMode.CBC: type = OpenSsl.EVP_aes_192_cbc(); break;
                        default:
                            throw new NotImplementedException();
                    }
                    break;
                case 32:
                    switch (parameters.Mode)
                    {
                        case BlockCipherMode.ECB: type = OpenSsl.EVP_aes_256_ecb(); break;
                        case BlockCipherMode.CBC: type = OpenSsl.EVP_aes_256_cbc(); break;
                        default:
                            throw new NotImplementedException();
                    }
                    break;
            ...
            if (OpenSsl.EVP_CipherInit_ex(_ctx, type, IntPtr.Zero, null, null, enc ? 1 : 0) != 1)
                throw new OpenSslException();

            int ivLength = OpenSsl.EVP_CIPHER_CTX_iv_length(_ctx);
            int keyLength = OpenSsl.EVP_CIPHER_CTX_key_length(_ctx);
            ...
            if (OpenSsl.EVP_CipherInit_ex(_ctx, IntPtr.Zero, IntPtr.Zero, key, parameters.IV, enc ? 1 : 0) != 1)
                throw new OpenSslException();
