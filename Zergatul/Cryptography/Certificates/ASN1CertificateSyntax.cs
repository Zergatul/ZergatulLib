...
namespace Zergatul.Cryptography.Certificates
{
    ...
    internal class ASN1CertificateSyntax
    {
        ...
        public static ASN1CertificateSyntax FromASN1Element(ASN1Element element)
        {
            if (element is Sequence)
            {
                ...
                if (certificate.Elements.Count == 3)
                {
                    ...
                    var sa = ParseAlgorithmIdentifier(certificate.Elements[1]);
        ...
        private static TBSCertificate ParseTBSCertificate(ASN1Element element)
        {
            if (element is Sequence)
            {
                ...
                if (7 <= tbs.Elements.Count && tbs.Elements.Count <= 10)
                {
                    ...
                    var sign = ParseAlgorithmIdentifier(tbs.Elements[index++]);
        ...
        private static AlgorithmIdentifier ParseAlgorithmIdentifier(ASN1Element element)
        {
            if (element is Sequence)
            {
                var ai = (Sequence)element;
                if (ai.Elements.Count >= 2)
                {
                    var algo = ai.Elements[0] as ObjectIdentifier;
                    var ps = new ASN1Element[ai.Elements.Count - 1];
                    for (int i = 0; i < ps.Length; i++)
                        ps[i] = ai.Elements[i + 1];
                    if (algo != null)
        ...
        private static SubjectPublicKeyInfo ParseSubjectPublicKeyInfo(ASN1Element element)
        {
            if (element is Sequence)
            {
                ...
                {
                    var algo = ParseAlgorithmIdentifier(spki.Elements[0]);
