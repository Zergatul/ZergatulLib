...
namespace Zergatul.Network.Tls
{
    ...
    // * client certificates
    public class TlsStreamNew : Security.Tls.TlsStream
    {
        ...
        public TlsStreamNew(Stream innerStream)
        {
        ...
        public override void AuthenticateAsClient()
        {
            ...
            {
                // todo add break
                if (_stateMachine.IsReading)
                {
                    continue;
                }
                if (_stateMachine.IsWriting)
                {
                    switch (_stateMachine.WState)
                    {
                        case StateMachine.WriteState.PreProcess:
                            break;
                        case StateMachine.WriteState.Write:
                            break;
                        case StateMachine.WriteState.Flush:
                            break;
                        case StateMachine.WriteState.PostProcess:
                            break;
                    }
                    continue;
                }
        ...
        private void Init()
        {
            ...
            SecurityParameters.HandshakeBuffer = new List<byte>();

            _messageStream = new RecordMessageStream(_stream, SecurityParameters.HandshakeBuffer);

            ConnectionInfo = new ConnectionInfo();

            _stateMachine = new StateMachine(this);
            if (_role == Role.Client)
                _stateMachine.InitClient();
            if (_role == Role.Server)
                _stateMachine.InitServer();
        ...
        private CipherSuite[] GetCipherSuites()
        {
            var cipherSuites = Parameters.CipherSuites ?? TlsDefaults.CipherSuites;
            if (cipherSuites == null)
                throw new TlsStreamException("Cannot create ciphersuites list");
            if (cipherSuites.Length == 0)
                throw new TlsStreamException("Ciphersuites list is empty");
            return cipherSuites;
        }
        ...
        private void WriteHandshakeMessageBuffered(HandshakeMessage message)
        {
            ...
            OnContentMessage(this, new ContentMessageEventArgs(message, false, _role == Role.Server));
        }
        ...
        internal ClientHello GenerateClientHello()
        {
            if (string.IsNullOrEmpty(Parameters.Host))
                throw new TlsStreamException("Parameters.Host is null or empty");
            ...
            ProtocolVersion version = ProtocolVersion.Tls13;
            if (Parameters.MaxVersion != null)
                version = (ProtocolVersion)TlsHelper.ToProtocolVersion(Parameters.MaxVersion.Value);

            // record layer should be TLS 1.0
        ...
        {
            var commonCiphers = GetCipherSuites()
        ...
        private void OnServerHello(ServerHello message)
        {
            ...
            SelectedCipher.Init(SecurityParameters, Settings, _role, _random);
