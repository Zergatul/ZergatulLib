...
namespace Zergatul.Security.Tests.Signature
{
    ...
    public class ECDSATests
    {
        ...
        private static void Test(string curve, string hash, string private_key, string pubkey_x, string pubkey_y, string k, string plain, string r, string s)
        {
            ...
            {
                // sign
                using (var signature = provider.GetSignature(Signatures.ECDSA))
                using (var md = provider.GetMessageDigest(hash))
                {
                    signature.InitForSign(new RawPrivateKey(BitHelper.HexToBytes(private_key)), new ECDSASignatureParameters
                    {
                        Curve = curve,
                        MessageDigest = md,
                        Random = new StaticSecureRandom(BitHelper.HexToBytes(k))
                    });
                    byte[] data = Encoding.ASCII.GetBytes(plain);
                    signature.Update(data, 0, data.Length);
                    byte[] encodedSignature = signature.Sign();

                    var ed = ECDSASignatureValue.Parse(Asn1Element.ReadFrom(encodedSignature));
                    var rBytes = ed.r.ToBytes(ByteOrder.BigEndian);
                    var sBytes = ed.s.ToBytes(ByteOrder.BigEndian);
                    Assert.IsTrue(BitHelper.BytesToHex(rBytes) == r);
                    Assert.IsTrue(BitHelper.BytesToHex(sBytes) == s);
                }
                ...
                using (var md = provider.GetMessageDigest(hash))
                {
                    signature.InitForVerify(new RawPublicKey(BitHelper.HexToBytes("04" + pubkey_x + pubkey_y)), new ECDSASignatureParameters
                    {
                        Curve = curve,
                        MessageDigest = md
                    });
                    byte[] data = Encoding.ASCII.GetBytes(plain);
                    signature.Update(data, 0, data.Length);
                    var ed = new ECDSASignatureValue(Math.BigInteger.Parse(r, 16), Math.BigInteger.Parse(s, 16));
                    Assert.IsTrue(signature.Verify(ed.ToBytes()));
                }
                ...
                using (var md = provider.GetMessageDigest(hash))
                {
                    ...
                    signature.Update(data, 0, data.Length);
                    var ed = new ECDSASignatureValue(Math.BigInteger.Parse(r, 16), Math.BigInteger.Parse(s, 16));
                    var encodedSignature = ed.ToBytes();
                    encodedSignature[encodedSignature.Length - 1] ^= 1;
                    Assert.IsFalse(signature.Verify(encodedSignature));
                }
            }
        }
    }
