...
namespace Zergatul.Security.OpenSsl.Tls
{
    class TlsStream : Security.TlsStream
    {
        ...
        private Native.VerifyCallbackDelegate _verifyCallback;
        ...
        public override void AuthenticateAsClient()
        {
            _isServer = false;
            Init();
            int ret = Native.SSL_connect(_ssl);
            if (ret != 1)
            {
                ProcessError(ret);
                return;
        ...
        public override void AuthenticateAsServer()
        {
            _isServer = true;
            Init();
            int ret = Native.SSL_accept(_ssl);
            if (ret != 1)
            {
                ProcessError(ret);
                return;
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                if (_stream != null)
                {
                    int ret = Native.SSL_shutdown(_ssl);
                    if (ret < 0)
                        ProcessError(ret);
                    if (Parameters.BidirectionalShutdown && Parameters.KeepOpen && ret == 0)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                ret = Native.SSL_read(_ssl, handle.AddrOfPinnedObject(), buffer.Length);
                                if (ret < 0)
                                    ProcessError(ret);
                            }
                        }
            ...
            {
                Native.SSL_CTX_free(_sslContext);
        ...
        public override void Flush()
        {
            ...
            if (_writeBufferPos != 0)
            {
                ...
                _writeBufferPos = 0;
                int ret = Native.SSL_write(_ssl, _writeBufferHandle.AddrOfPinnedObject(), count);
                _writeBufferPos = 0;
                if (ret <= 0)
                    ProcessError(ret);
                if (ret != count)
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            try
            {
                int ret = Native.SSL_read(_ssl, handle.AddrOfPinnedObject() + offset, count);
                if (ret <= 0)
                    ProcessError(ret);
                return ret;
            }
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            ...
            while (count > 0)
            {
                ...
                {
                    int ret = Native.SSL_write(_ssl, _writeBufferHandle.AddrOfPinnedObject(), _writeBuffer.Length);
                    _writeBufferPos = 0;
                    if (ret <= 0)
                        ProcessError(ret);
        ...
        private void Init()
        {
            ...
            IntPtr method = _isServer ? Native.TLS_server_method() : Native.TLS_client_method();
            _sslContext = Native.SSL_CTX_new(method);
            if (_sslContext == IntPtr.Zero)
                throw new OpenSslException();

            _ssl = Native.SSL_new(_sslContext);
            if (_ssl == IntPtr.Zero)
                throw new OpenSslException();
            ...
            {
                if (Native.SSL_set_min_proto_version(_ssl, TlsHelper.ToProtocolVersion(Parameters.MinVersion.Value)) != 1)
                    throw new OpenSslException();
            }

            if (Parameters.MaxVersion != null)
            {
                if (Native.SSL_set_max_proto_version(_ssl, TlsHelper.ToProtocolVersion(Parameters.MaxVersion.Value)) != 1)
                    throw new OpenSslException();
            }
            ...
            Native.SSL_set_bio(_ssl, _wrapper.Bio, _wrapper.Bio);
            ...
            if (_isServer)
            {
                if (Parameters.RequestClientCertificate == true)
                    Native.SSL_set_verify(_ssl, Native.SSL_VERIFY_PEER, _verifyCallback);
                else
                    Native.SSL_set_verify(_ssl, Native.SSL_VERIFY_NONE, null);
            }
            else
            {
                Native.SSL_set_verify(_ssl, Native.SSL_VERIFY_PEER, _verifyCallback);
        ...
        private int CertificateValidateCallback(int preverifyOk, IntPtr x509Ctx)
        {
            ...
            else
            {
                ...
                if (validate != null)
                {
                    IntPtr x509 = Native.X509_STORE_CTX_get_current_cert(x509Ctx);
                    if (x509 == IntPtr.Zero)
                        throw new InvalidOperationException();
                    IntPtr ptr = IntPtr.Zero;
                    int length = Native.i2d_X509(x509, ref ptr);
                    if (length < 0)
                        throw new OpenSslException();
                    ...
                    try
                    {
                        ...
                        if (Native.i2d_X509(x509, ref ptr) != length)
                            throw new InvalidOperationException();
                    }
                    finally
                    {
        ...
        private void OnAuthFinished()
        {
            if (_writeBuffer == null)
            {
                _writeBuffer = new byte[TlsHelper.PlaintextLimit];
        ...
        {
            return (Native.SSL_get_shutdown(_ssl) & Native.SSL_RECEIVED_SHUTDOWN) != 0;
        }

        private void ProcessError(int ret)
        {
            switch (Native.SSL_get_error(_ssl, ret))
            {
                ...
                case Native.SSL_ERROR_WANT_X509_LOOKUP:
                ...
                case Native.SSL_ERROR_WANT_CLIENT_HELLO_CB:
