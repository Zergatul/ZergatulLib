...
namespace Zergatul.Security.OpenSsl.Tls
{
    internal class BioStreamWrapper : IDisposable
    {
        ...
        public BioStreamWrapper(Stream stream, int bufferSize = 0x8000)
        {
            ...
            Bio = Native.BIO_new(_bioMethod);
            if (Bio == IntPtr.Zero)
                throw new OpenSslException();

            Native.BIO_set_init(Bio, 1);
            Native.BIO_set_data(Bio, (IntPtr)_id);
        ...
        private static readonly Native.BIOCtrlDelegate CtrlDelegate;
        private static readonly Native.BIOReadDelegate ReadDelegate;
        private static readonly Native.BIOWriteDelegate WriteDelegate;

        static BioStreamWrapper()
        {
            int type = Native.BIO_get_new_index();
            if (type == -1)
                throw new OpenSslException();

            _bioMethod = Native.BIO_meth_new(type, nameof(BioStreamWrapper));
            if (_bioMethod == IntPtr.Zero)
                throw new OpenSslException();

            CtrlDelegate = BIOCtrl;
            if (Native.BIO_meth_set_ctrl(_bioMethod, CtrlDelegate) != 1)
                throw new OpenSslException();

            ReadDelegate = BIORead;
            if (Native.BIO_meth_set_read(_bioMethod, ReadDelegate) != 1)
                throw new OpenSslException();

            WriteDelegate = BIOWrite;
            if (Native.BIO_meth_set_write(_bioMethod, WriteDelegate) != 1)
                throw new OpenSslException();
        ...
        private static long BIOCtrl(IntPtr bio, int cmd, long larg, IntPtr parg)
        {
            switch (cmd)
            {
                case Native.BIO_CTRL_PUSH:
                case Native.BIO_CTRL_POP:
                    return 0;

                case Native.BIO_CTRL_FLUSH:
        ...
        {
            int id = (int)Native.BIO_get_data(bio);
            lock (_syncRoot)
                return _wrappers[id];
        }
        ...
        private class StaticFinalizer
        {
            ~StaticFinalizer()
            {
                Native.BIO_meth_free(_bioMethod);
            }
        }
