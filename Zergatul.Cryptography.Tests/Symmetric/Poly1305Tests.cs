...
namespace Zergatul.Cryptography.Tests.BlockCipher
{
    ...
    public class Poly1305Tests
    {
        ...
        public void Vector1()
        {
            ...
            byte[] nonce = new byte[] { 0x07, 0x00, 0x00, 0x00 }.Concat(iv).ToArray();
            ...
            TestEncryptDecrypt(plain, aad, key, nonce, ciphertext, tag);
        }
        ...
        private static void TestEncryptDecrypt(byte[] plain, byte[] aad, byte[] key, byte[] nonce, byte[] ciphertext, byte[] tag)
        {
            var poly1305 = new Poly1305();
            var generator = new Poly1305KeyGenerator();
            var enc = poly1305.CreateEncryptor(new ChaCha20(), key, generator.GenerateKey(key, nonce));
            var encData = enc.Encrypt(nonce, plain, aad);

            Assert.IsTrue(encData.CipherText.SequenceEqual(ciphertext));
            Assert.IsTrue(encData.Tag.SequenceEqual(tag));

            var dec = poly1305.CreateDecryptor(new ChaCha20(), key, generator.GenerateKey(key, nonce));
            var decData = dec.Decrypt(nonce, encData, aad);

            Assert.IsTrue(decData.SequenceEqual(plain));
        }
    }
}